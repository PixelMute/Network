...
// Sweetsparse library?
// Eigen library?
// Want a library for sparse matrix multiplication.
...
std::vector<double> loadWavFile(const std::string& filename) {
    ...
    if (!sndfile) {
        ...
        return std::vector<double>(); // Return an empty vector on failure
    }

    // Read the audio data
    const sf_count_t numFrames = sfinfo.frames;
    std::vector<double> audioData(numFrames);
            ...
            int input_size, int output_size, int kernel_size, int input_channels, int output_channels, int stride)
{
    ...
    if (bias != NULL)
    {
      ...
      {
        output[i] = bias[i % output_channels];
      }
    }
...
void conv1dTranspose(double* input, double* kernel, double* output,
    ...
    { 
      int i_prime = i * stride; // index in output
      for (k = 0; k < kernel_size; k++)
      {
        
        output[i_prime + k] += input[i] * kernel[k];
        //std::cout << "Index " << i_prime + k << " is now " << output[i_prime + k] << " from i=" << i << ", k=" << k << std::endl;
      }
    }
}
            ...
            int input_size, int kernel_size, int input_channels, int output_channels, int stride)
{
    ...
    //std::cout << "output_size: " << output_size << ". Kernel_size: " << kernel_size << std::endl;
    if (bias != NULL)
    {
      for (i = 0; i < output_size * output_channels; i++) 
      {
    ...
    //print_array(output, 0, output_size * output_channels, "output");
    ...
    for (i = 0; i < input_channels; i++)
    {
        ...
        for (j = 0; j < output_channels; j++)
        {
            ...
            double* current_inp_channel = &input[i * input_size];
            double* current_weight = &weight[j * kernel_size + i * kernel_size * output_channels];
            double* channel_out = &output[j * output_size];
            //double* channel_out2 = (double*) malloc(output_size * sizeof(double));
            
            //std::cout << "input size: " << input_size << ", kernel size: " << kernel_size << ", output size: " << output_size << ", stride: " << stride << std::endl;
            //print_array(current_inp_channel, 0, input_size, "input");
            //print_array(current_weight, 0, kernel_size, "weight");
            conv1dTranspose(current_inp_channel, current_weight, channel_out, input_size, kernel_size, output_size, stride);

            /*for (int k = 0; k < output_size; k++)
            {
              std::cout << "Old value: " << channel_out2[k] << ". New value: " << channel_out[k] << ". Change: " << channel_out[k] - channel_out2[k] << std::endl;
            }
            print_array(output, 0, output_size * output_channels, "Full output");
            free(channel_out2);*/
        }
    }
}

// Placeholder in case we need more complicated logic later.
// This is where we would load the weights.
...
void runDenoiserExperiments(DenoiserState* ds, WorkingMemory* wm, DenoiserStatePyTorch* dspt)
{
  ...
  for (sparsity = 0; sparsity < 100; sparsity += step)
  {
    for (int i=0; i < iterations; i++)
    {
      ...
      //}
    std::string filenum = std::to_string(i % 5 + 1);
    std::string filename = filenum + ".wav";
    std::vector<double> audioData = loadWavFile(filename);
...
int main(int argc, char *argv[]) {
  ...
  if (operation == 0)
  {
    ...
    if (argc > 2)
    {
      ...
      std::vector<double> audioData = loadWavFile(argv[2]);
  ...
  else if (operation == 3)
  {
    ...
    int input_size = 5;
    int kernel_size = 3;
    double* inp = (double*) malloc(input_size * input_channels * sizeof(double));
    for (int j = 0; j < input_channels; j++)
    {
      for (int i=0; i < input_size; i++) 
      {
          inp[j * input_size + i] = sin(i * .1 + j);
      }
    }
    double* kernel = (double*) malloc(input_channels * output_channels * kernel_size * sizeof(double));
    for (int i=0; i < input_channels; i++) 
    {
      for (int j=0; j < output_channels; j++)
      {
        ...
        {
            kernel[i * output_channels * kernel_size + j * kernel_size + k] = sin(k * -.1 + i + j);
        }
      }
    }
    int output_size = (input_size - 1) * 2 + kernel_size;
    double* output = (double*) malloc(output_size * output_channels * sizeof(double));
    for (int i=0; i < ((5 - 1) * 2 + 3) * 4; i++) 
    {
        output[i] = 0;
    }

    double* bias = (double*) malloc(output_channels * sizeof(double));
    for (int i=0; i < output_channels; i++) 
    {
        bias[i] = 0;
    }

    conv1dTransposeChannels(inp, kernel, bias, output, input_size, kernel_size, input_channels, output_channels, 2);
